...
<body class="with-watermark">
    ...
    <table>
        ...
        <tbody>
            @foreach ($data as $row)
                @php
                    $statusClass = match($row->status_simpanan) {
                        'Masih Ada Simpanan' => 'status-selesai',
                        'Direncanakan' => 'status-direncanakan',
                        'Tidak Ada Simpanan (Sudah Ditarik Semuanya)' => 'status-dibatalkan',
                        default => 'status-lainnya'
                    };

                    // Ambil hanya angka dari RW
                    $rw = preg_replace('/[^0-9]/', '', $row->asal_rt_rw_simpanan);
                @endphp
                <tr>
                    ...
                    <td class="{{ $statusClass }}">{{ $row->status_simpanan }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
