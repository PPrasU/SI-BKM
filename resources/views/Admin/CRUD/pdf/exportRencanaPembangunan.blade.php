...
<body class="with-watermark">
    ...
    <h2>Pembangunan Dengan Lembaga Swasta</h2>
        ...
        <tbody>
            @foreach ($data as $row)
                @php
                    ...
                    $rw = preg_replace('/[^0-9]/', '', $row->rw_rencana_pemb);
                @endphp
                <tr>
                    <td>{{ $rw }}</td>
                    <td class="justify">{{ $row->rencana_pembangunan_dengan }}</td>
                    <td class="justify">{{ $row->detail_pemb }}</td>
                    <td>
                        @php
